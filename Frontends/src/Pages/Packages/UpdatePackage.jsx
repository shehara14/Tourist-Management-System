...
const UpdateTourPackage = () => {
  ...
  useEffect(() => {
    const fetchPackageData = async () => {
      try {
        ...
        // Transform places data to match our state structure
        const transformedPlaces = packageData.places.map(place => ({
          ...place,
          image: place.images?.[0] || '',
          images: place.images || [],
          suitableFor: {
            ...place.suitableFor,
            climate: place.suitableFor?.climate || 'Tropical',
            ageRange: place.suitableFor?.ageRange || { min: 0, max: 100 },
            hobbies: place.suitableFor?.hobbies || [],
            healthConsiderations: place.suitableFor?.healthConsiderations || {
              notRecommendedFor: [],
              specialFacilities: []
            }
          },
  ...
  const validatePlace = () => {
    ...
    if (!currentPlace.image && currentPlace.images.length === 0) newErrors.images = "At least one image is required.";
      ...
      <Box display="flex" style={{ backgroundColor: '#f5f5f5'}}>
        ...
        >
          ...
          <Box component="form" noValidate autoComplete="off" onSubmit={handleSubmit}>
            <Grid container spacing={3}>
              ...
              <Grid item xs={12} md={6} style={{padding:'30px'}}>
                ...
                {places.length === 0 ? (
                  ...
                  <Box sx={{ maxHeight: '880px', overflow: 'auto' }}>
                    ...
                    <Grid container spacing={2}>
                      ...
                      <TextField
                        ...
                        onChange={(e) => {
                          ...
                          // For negative values, we need to handle differently since we're removing the minus sign
                          const finalValue = e.target.value.startsWith('-') ? -Math.abs(value) : value;
                          
                          // Validate latitude between -90 and 90 degrees
                          const validValue = isNaN(finalValue) ? '' : Math.min(Math.max(-90, finalValue), 90);
                        ...
                        // Use onKeyDown to prevent typing + and - directly
                        onKeyDown={(e) => {
                          if (e.key === '+' || e.key === '-') {
                            e.preventDefault();
                          }
                      ...
                      <TextField
                        ...
                        onChange={(e) => {
                          ...
                          const validValue = isNaN(finalValue) ? '' : Math.min(Math.max(-180, finalValue), 180);
            ...
            <Grid container spacing={2} mt={4} style={{marginTop:'50px'}}>
              <Grid item xs={6}>
                <Button
                  fullWidth
                  variant="contained"
